    ...
    I_client_renderable* pRenderable, void* pBonesOut, int nMaxBones, int iMask, int iCurTime)
{
    ...
    // weapon is drawn using bones that are calculated in its own setupbones call ( 7 to 8 frames ), else is uses its owner's bone matrix maybe.
    //

    // Check if we need to bone matrix for custom real angles?
    if (F::tickManipHelper.ModifyRealBoneAngles() == false)
