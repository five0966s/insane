...
void Movement_t::Run(BaseEntity* pLocalPlayer, baseWeapon* pActiveWeapon, CUserCmd* pCmd, bool& result)
{
    ...
    _RocketJumpAssist(pLocalPlayer, pActiveWeapon, pCmd, result);
...
///////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////
void Movement_t::_RocketJumpAssist(BaseEntity* pLocalPlayer, baseWeapon* pActiveWeapon, CUserCmd* pCmd, bool& bCreateMoveResult)
{
    if (Features::Movement::RocketJumpAssist::RocketJumpAssist.IsActive() == false)
        return;
    ...
    float flRocketJumpHeight = Features::Movement::RocketJumpAssist::RocketJumpAssist_RJHeight.GetData().m_flVal;


    // Place where we shoot our rockets to Rocket Jump.
    vec vRJTargetPos   (0.0f);
    vec vTraceEnd      (0.0f);
    vec vExpectedOrigin(0.0f);

    // How behind are we shooting the rocket compared to our abs. origin.
    float flRJPosOffset = 0.0f;

    // Time required to reach the desired height from the ground so we can RJ.
    int iTicksToReachRJPos = -1;

    if (F::movementSimulation.Initialize(pLocalPlayer, true) == true)
    {
        ITraceFilter_IgnoreSpawnVisualizer traceFilter(pLocalPlayer); trace_t trace;

        for (int iTick = 0; iTick < 64; iTick++)
        {
            F::movementSimulation.RunTick();
            vec vSimulatedOrigin = F::movementSimulation.GetSimulationPos();
            vec vHellPos(vSimulatedOrigin.x, vSimulatedOrigin.y, vSimulatedOrigin.z - 200.0f);
            I::EngineTrace->UTIL_TraceRay(vSimulatedOrigin, vHellPos, MASK_SHOT, &traceFilter, &trace);

            if (vSimulatedOrigin.DistTo(trace.m_end) <= flRocketJumpHeight)
            {
                qangle qEngineAngles; I::iEngine->GetViewAngles(qEngineAngles);
                vec    vForward;      Maths::AngleVectors(qEngineAngles, &vForward);
                vForward.NormalizeInPlace();
                

                float flRocketOffsetMin = Features::Movement::RocketJumpAssist::RocketJumpAssist_OffsetMin.GetData().m_flVal;
                float flRocketOffsetMax = Features::Movement::RocketJumpAssist::RocketJumpAssist_OffsetMax.GetData().m_flVal;


                // If player is on ground, i.e. ain't rocket jumping or failed rocket jump
                // then simple rocket jump no bullshit.
                if (pLocalPlayer->m_fFlags() & FL_ONGROUND)
                {
                    vRJTargetPos = pLocalPlayer->GetAbsOrigin() + (vForward * flRocketOffsetMax * -1.0f);
                }
                else
                {
                    vRJTargetPos        = vSimulatedOrigin;

                    vec   vSimulatedVel = F::movementSimulation.GetSimulationVel();
                    float flOffsetMult  = std::clamp<float>(fabsf(vSimulatedVel.z) / 400.0f, 0.0f, 1.0f);
                    flRJPosOffset       = flRocketOffsetMax - (flOffsetMult * (flRocketOffsetMax - flRocketOffsetMin));
                    vRJTargetPos        = trace.m_end - (vForward * flRJPosOffset);

                    I::EngineTrace->UTIL_TraceRay(vRJTargetPos, vHellPos, MASK_SHOT, &traceFilter, &trace);
                    vRJTargetPos.z      = trace.m_end.z; // We only need the Z coords, so we will only set Z.
                }

                vExpectedOrigin    = vSimulatedOrigin;
                vTraceEnd          = trace.m_end;
                iTicksToReachRJPos = iTick;
    ...
    // We can't reach RJ pos.
    if (iTicksToReachRJPos < 0)
        return;

    vec vEyePos = pLocalPlayer->GetEyePos();
    qangle qRJAngles; Maths::VectorAnglesFromSDK(vRJTargetPos - vEyePos, qRJAngles);
    ProjectileInfo_t& projInfo = F::projectileEngine.SetupProjectile(pActiveWeapon, pLocalPlayer, qRJAngles);

    // Rocket should reach on the tick next to when we reach target pos.
    int iRocketTicksToReach  = TIME_TO_TICK(vEyePos.DistTo(vRJTargetPos) / projInfo.m_flSpeed);
    if (iRocketTicksToReach <= iTicksToReachRJPos)
        return;

    // Now, the rocket will reach the RJ target pos at roughly the same time as us.
    if (SDK::CanAttack(pLocalPlayer, pActiveWeapon) == true)
    {
        pCmd->buttons    |= IN_ATTACK;
        pCmd->viewangles  = qRJAngles;
        bCreateMoveResult = false;


#define DEBUG_ROCKETJUMP_ASSIST false
#if (DEBUG_ROCKETJUMP_ASSIST == true)

        /*
        -> When coming down too fast, we shoot 20 units behind us, that would not give us enough upward velocity
        so we can fire again when we touch the ground next time. 
        -> We need to shoot such that we get enough time before reaching the ground again that our weapon can shoot again
        -> We need to check if velocity.z -> -300, our offset should approach near 0 so we can add more upward velocity.
        */

        vec vOriginNow = pLocalPlayer->GetAbsOrigin();

        I::IDebugOverlay->ClearAllOverlays();
        I::IDebugOverlay->AddBoxOverlay(vEyePos,      vec(2.0f), vec(-2.0f), qangle(0.0f),   0, 255,   0, 255, 5.0f);
        I::IDebugOverlay->AddBoxOverlay(vOriginNow,   vec(2.0f), vec(-2.0f), qangle(0.0f),   0, 255,   0, 255, 5.0f);
        I::IDebugOverlay->AddBoxOverlay(vRJTargetPos, vec(2.0f), vec(-2.0f), qangle(0.0f), 255,   0,   0, 255, 5.0f);
        I::IDebugOverlay->AddBoxOverlay(vTraceEnd,    vec(2.0f), vec(-2.0f), qangle(0.0f), 255,   0, 255, 255, 5.0f);
        I::IDebugOverlay->AddBoxOverlay(vExpectedOrigin, vec(2.0f), vec(-2.0f), qangle(0.0f), 255, 255,   0, 255, 5.0f);

        I::IDebugOverlay->AddTextOverlay(vTraceEnd,       5.0f, "Trace End");
        I::IDebugOverlay->AddTextOverlay(vRJTargetPos,    5.0f, "Rocket");
        I::IDebugOverlay->AddTextOverlay(vExpectedOrigin, 5.0f, "Expected Origin");

        
        I::IDebugOverlay->AddLineOverlay(vEyePos, vOriginNow, 255, 255, 255, true, 5.0f);


        F::movementSimulation.Initialize(pLocalPlayer, true);
        for (int iTick = 0; iTick < iRocketTicksToReach; iTick++)
            F::movementSimulation.RunTick();

        I::IDebugOverlay->AddBoxOverlay(F::movementSimulation.GetSimulationPos(), vec(4.0f), vec(-4.0f), qangle(0.0f), 255, 255, 255, 255, 5.0f);
        I::IDebugOverlay->AddTextOverlay(F::movementSimulation.GetSimulationPos(), 5.0f, "Expected @ explosion");

        F::movementSimulation.Restore();


        LOG("Distance from ground : %.2f | Z Velocity : %.2f | Offset : %.2f | PING { %d }", 
            vOriginNow.z - vRJTargetPos.z, pLocalPlayer->m_vecAbsVelocity().z, flRJPosOffset);
#endif
    }
}
