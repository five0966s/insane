...
void EntityIterator_t::Run(BaseEntity* pLocalPlayer, baseWeapon* pActiveWeapon, CUserCmd* pCmd)
{
    ...
    for (int iEntIndex = 0; iEntIndex < nEntities; iEntIndex++)
    {
        ...
        if (iClassID == ClassIDStatic_t::ClassIDStatic_CTFPlayer && vAllConnectedEnemies != nullptr && vAllConnectedTeammates != nullptr)
        {
        ...
        case ClassIDStatic_t::ClassIDStatic_CObjectSentrygun:  _ProcessSentry    (pEnt, pLocalPlayer->m_iTeamNum()); break;
        case ClassIDStatic_t::ClassIDStatic_CObjectDispenser:  _ProcessDispenser (pEnt, pLocalPlayer->m_iTeamNum()); break;
        case ClassIDStatic_t::ClassIDStatic_CObjectTeleporter: _ProcessTeleporter(pEnt, pLocalPlayer->m_iTeamNum()); break;
        case ClassIDStatic_t::ClassIDStatic_CTFStickBomb:      _ProcessPipeBomb  (pEnt, pLocalPlayer->m_iTeamNum()); break;
        default: break;
        }
    }
