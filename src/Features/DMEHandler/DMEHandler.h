...
private:
    ...
    int64_t _HandleLocalPlayer     (void* pVTable, DrawModelState_t* modelState, ModelRenderInfo_t* renderInfo, matrix3x4_t*  pBoneMatrix, void* pOriginalDME);
    ...
    // Handling glow...
    int64_t _HandleGlowModels      (void* pVTable, DrawModelState_t* modelState, ModelRenderInfo_t* renderInfo, matrix3x4_t* boneMatrix, void* pOriginalDME);
    int64_t _DrawBackTrackGlow     (void* pVTable, DrawModelState_t* modelState, ModelRenderInfo_t* renderInfo, matrix3x4_t* boneMatrix, void* pOriginalDME, BaseEntity* pEnt);
    void    _DrawAntiAimGlow       (void* pVTable, DrawModelState_t* modelState, ModelRenderInfo_t* renderInfo, matrix3x4_t* boneMatrix, void* pOriginalDME, BaseEntity* pEnt);
...
DEFINE_FEATURE(BackTrack_Cham_Setting,     "BackTrack Chams Type",  DropDown_t, BackTrack, BackTrack, 2, DropDown_t(pBackTrackChamOptions, 3))
DEFINE_FEATURE(BackTrack_Cham,          "BackTrack Cham Material",  DropDown_t, BackTrack, BackTrack, 3, DropDown_t(pDummyList, 1));
DEFINE_FEATURE(BackTrack_GlowSetting,       "BackTrack Glow Type",  DropDown_t, BackTrack, BackTrack, 4, DropDown_t(pBackTrackChamOptions, 3))
DEFINE_FEATURE(BackTrack_GlowClr,            "BackTrack Glow Clr", ColorData_t, BackTrack, BackTrack, 5, ColorData_t((unsigned char)255, 255, 255, 255))
DEFINE_FEATURE(BackTrack_GlowDrawFrequency, "Glow Draw Frequency", IntSlider_t, BackTrack, BackTrack, 6, IntSlider_t(1, 1, 10))
